...
def prune_model(model, example_input, iterative_steps=5, backbone_sparsity=0.10, head_sparsity=0.2):
    """
    ...
    importance_criterion = tp.importance.TaylorImportance()  # Or tp.importance.MagnitudeImportance()
    ...
    for step in range(iterative_steps):
        if isinstance(importance_criterion, tp.importance.TaylorImportance):
            # TaylorImportance requires gradients for importance estimation
            output = model(example_input)  # Output is a tuple
            loss = output[0].sum()  # Use the first element of the tuple
            loss.backward()  # Compute gradients before pruner.step()
        pruner.step()
