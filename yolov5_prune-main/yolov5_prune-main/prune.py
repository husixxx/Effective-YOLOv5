...
def prune(data,
    ...
    model.eval()
    debug_layer_channels(model, "BEFORE PRUNING")
    # prune model start
...
def debug_layer_channels(model, name="Original"):
    """Debug function to print all layer dimensions"""
    print(f"\n=== {name} MODEL CHANNEL DIMENSIONS ===")
    for i, m in enumerate(model.model):
        if hasattr(m, 'conv') and hasattr(m.conv, 'weight'):
            in_ch = m.conv.weight.shape[1]
            out_ch = m.conv.weight.shape[0]
            print(f"Layer {i} ({m.type}): in={in_ch}, out={out_ch}")
        elif hasattr(m, 'm') and isinstance(m.m, nn.ModuleList):  # C3 modules
            print(f"Layer {i} ({m.type}):")
            if hasattr(m, 'cv1') and hasattr(m.cv1, 'conv'):
                print(f"  cv1: in={m.cv1.conv.weight.shape[1]}, out={m.cv1.conv.weight.shape[0]}")
            if hasattr(m, 'cv2') and hasattr(m.cv2, 'conv'):
                print(f"  cv2: in={m.cv2.conv.weight.shape[1]}, out={m.cv2.conv.weight.shape[0]}")
            for j, bottleneck in enumerate(m.m):
                if hasattr(bottleneck, 'cv1') and hasattr(bottleneck.cv1, 'conv'):
                    print(f"  bottleneck {j} cv1: in={bottleneck.cv1.conv.weight.shape[1]}, out={bottleneck.cv1.conv.weight.shape[0]}")
                if hasattr(bottleneck, 'cv2') and hasattr(bottleneck.cv2, 'conv'):
                    print(f"  bottleneck {j} cv2: in={bottleneck.cv2.conv.weight.shape[1]}, out={bottleneck.cv2.conv.weight.shape[0]}")
