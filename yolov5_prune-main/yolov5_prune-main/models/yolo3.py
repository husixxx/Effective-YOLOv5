...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            x = m(x)  # run
            # print(f"Layer {m.i} input shape: {x.shape if isinstance(x, torch.Tensor) else [xi.shape for xi in x]}")
            # if isinstance(m, Conv):  # Apply ECA after convolutional layers
            #     if not hasattr(m, 'eca'):
            #         eca_layer = ECALayer(channel=x.shape[1])
                # x = eca_layer(x)
            # print(f"Vrstva {m.i}: {type(m).__name__}, Výstup: {x.shape}")
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, CBL}:
