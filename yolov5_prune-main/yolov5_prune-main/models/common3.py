...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"Vstup do Conv: {x.shape}")
        x = self.conv(x)
        x = self.bn(x)
        x = self.act(x)
        print(f"Výstup z Conv: {x.shape}")
        return x
...
class ResUnit(nn.Module):
    """
    Residual Unit (ResUnit) for CSP1_X.
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.cbl1 = CBL(in_channels, out_channels, 1, 1)  # 1x1 CBL
        self.cbl2 = CBL(out_channels, out_channels, 3, 1, padding=1)  # 3x3 CBL
...
class CSP1_X(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """
        CSP1_X Block for YOLOv5.
        
        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of Residual Units (ResUnits).
        ...
        # Initial CBL
        self.cbl1 = CBL(c1, c_, 1, 1)  # CBL for main branch
        self.cbl2 = CBL(c1, c_, 1, 1)  # CBL for shortcut branch
        
        # Residual Units
        self.m = nn.Sequential(*[ResUnit(c_, c_) for _ in range(n)])
        
        # Final CBL
        self.conv1 = nn.Conv2d(2 * c_, c2, 1, 1, bias=False)
        self.conv2 = nn.Conv2d(2 * c_, c2, 1, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(2 * c_)
        self.act = nn.ReLU()

    def forward(self, x):
        # Main branch: CBL -> ResUnits -> Conv
        y1 = self.cbl1(x)
        y1 = self.m(y1)
        y1 = self.conv(y1)
